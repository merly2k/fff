...
/*
foreach ($pp->get_result("select * from markers group by task") as $point) {
  $nameNode = $dom->createElement('name',htmlentities($row->name));
  $placeNode->appendChild($nameNode);
  $descNode = $dom->createElement('description', $row->address);
  $placeNode->appendChild($descNode);
...
foreach ($pps->get_result("Select * from markers where `task`='$point->task'") as $pointer):
  // Creates a coordinates element and gives it the value of the lng and lat columns from the results.
  $coorStr.= $pointer->lng . ','  . $pointer->lat."\r\n";
