...
if (!$_POST):
    $context = "<div class='col-md-12'>"
	    ...
	    . "      <span class='fileinput-new'>Выберите фото</span>"
	    ...
	    . "     <input class='form-control col-9' type='text' id='textinput' name='name'/>"
	    . "</div>"
	    . "<div class='width_full'>"
	    . "<textarea name='decription' id='editor1' class='editor1' rows='10' style='width:100%'></textarea>"
	    . "</div>"
    ...
    $filename = strtolower(translit($img_x . "X" . $img_y . "_" . $_FILES['userfile']['name']));

    $uploaddir = APP_PATH . DS . 'media' . DS . 'galery' . DS;
    $uploadfile = $uploaddir . DS . $filename;
    $tumbfile = $uploaddir . DS . 'tumb' . DS . $filename;
    $www_fail = WWW_MEDIA_PATH . 'galery/' . $filename;

    chmod($uploadfile, 0777); // в случае Unix хостинга возможно нужно 
    move_uploaded_file($_FILES['userfile']['tmp_name'], '' . $uploadfile);
    img_resize($uploadfile, $uploadfile, $img_x, $img_y, 70, 0xE7E2CE, 0);
    //img_resize($uploadfile, $tumbfile, $tumb_x, $tumb_y, 70, 0xE7E2CE, 1);
    $ph = new img_galery();
    $param=array(
	'name'=>"$name",
...
function uploadform($id) {
    ...
    echo "<h2>загрузка файлов</h2>
        <form enctype='multipart/form-data' action='" . WWW_BASE_PATH . "loader1' method='post'>
        <input type='hidden' name='id' value='$id' />
        <label>Фото: </label><input type='file' name='userfile' size='27'><br />
