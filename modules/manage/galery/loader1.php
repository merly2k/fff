...
if(!$_POST){
    ...
    extract($_POST);
    //print_r($_POST);
$filename = strtolower(translit("_".$_FILES['userfile']['name']));

$uploaddir=APP_PATH.DS.'images'.DS.'katalog'.DS;
$uploadfile = $uploaddir .DS. $filename;
$tumbfile=$uploaddir .DS.'tumb'.DS. $filename;
$www_fail=WWW_IMAGE_PATH.'katalog/'.$filename;

echo '<img src="'.$www_fail.'" />&nbsp;';
echo "<script>location.replace('".WWW_BASE_PATH."manage/pricingl/');</script>";
chmod($uploadfile,0777); // в случае Unix хостинга возможно нужно 
move_uploaded_file($_FILES['userfile']['tmp_name'], ''.$uploadfile);
img_resize($uploadfile, $uploadfile, 480, 480,  70, 0x000000, 0);
img_resize($uploadfile, $tumbfile, 280, 141,  70, 0xE7E2CE, 1);
}
