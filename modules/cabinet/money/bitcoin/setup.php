...
$result = mysqli_connect($mysql_host, $mysql_username, $mysql_password);
if (!$result) {
    die(__LINE__ . ' Invalid connect: ' . mysqli_error());
}

$result = mysqli_query('CREATE DATABASE IF NOT EXISTS `' . $mysql_database . '`');

if (!$result) {
    die(__LINE__ . ' Invalid query: ' . mysqli_error());
}

mysqli_select_db($mysql_database) or die( "Unable to select database. Run setup first.");
