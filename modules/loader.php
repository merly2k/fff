...
function uploadform($id){
    ...
    echo "<h2>загрузка файлов</h2>
        <form enctype='multipart/form-data' action='".WWW_BASE_PATH."loader' method='post'>
...
if(!$_POST){
    $ids=explode("/",$_GET[route]);
    $id=$ids[1];
    uploadform($id);
}else{
    extract($_POST);
    //print_r($_POST);
$filename = $id."_".$_FILES['userfile']['name'];

$uploaddir=APP_PATH.DS.'images'.DS.'article_img'.DS;
$tumbnail =APP_PATH.DS.'images'.DS.'article_img'.DS.'thumbnails'.DS.$filename;
$uploadfile = $uploaddir .DS. $filename;
$www_nail=WWW_IMAGE_PATH.'article_img/thumbnails/tumb_'.$filename;
$www_fail=WWW_IMAGE_PATH.'article_img/'.$filename;

echo '<img src="'.$www_fail.'" />&nbsp;';
echo "<script>location.replace('".WWW_BASE_PATH."my-store/edit/id/".$id."');</script>";
chmod($uploadfile,0777); // в случае Unix хостинга возможно нужно 
move_uploaded_file($_FILES['userfile']['tmp_name'], ''.$uploadfile);
img_resize($uploadfile, $uploadfile, 350, 200,  70, 0xFFFFF0, 0);
img_resize($uploadfile, $tumbnail, 150, 200,  70, 0xFFFFF0, 0);
